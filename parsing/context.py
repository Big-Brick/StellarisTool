...
@dataclass(frozen=True)
class StabilityResource(Resource):
	def __init__(self) -> None:
		super().__init__(key="stability", name="Stability")
...
class GlobalContext:
	...
	planet_job_upkeep_mult_re: re.Pattern[str] = re.compile(
		r'\bplanet_(?P<group>[A-Za-z0-9_]+)_upkeep_mult\s*=\s*(?P<amount>[-+]?\d+(?:\.\d+)?)'
	)
